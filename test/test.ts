...
describe('antlr4-c3:', function () {
  ...
  describe('Symbol table tests:', function () {
    it("Single table base tests", function (done) {
      ...
      try {
        ...
        if (e instanceof c3.DuplicateSymbolError) {
          expect(e.message, "Test 3").to.equal("Attempt to add duplicate symbol 'var3'");
        } else {
          assert(false);
        }
      }
      ...
      expect(variable!.getRoot(), "Test 10").to.equal(symbolTable);
      ...
      expect(variable!.getRoot(), "Test 12").to.equal(class1);
      ...
      expect(symbolTable, "Test 33").to.equal(symbolTable.getSymbolTable());
    ...
    it("Single table type checks", function (done) {
      ...
      expect(symbolTable.getAllNestedSymbols().length, "Test 1").to.equal(31);
      expect(symbolTable.getNestedSymbolsOfType(c3.Symbol).length, "Test 2").to.equal(31);
      expect(symbolTable.getNestedSymbolsOfType(c3.ScopedSymbol).length, "Test 3").to.equal(12);
      expect(symbolTable.getNestedSymbolsOfType(c3.TypedSymbol).length, "Test 4").to.equal(8);
      expect(symbolTable.getNestedSymbolsOfType(c3.RoutineSymbol).length, "Test 5").to.equal(3);
      expect(symbolTable.getNestedSymbolsOfType(c3.VariableSymbol).length, "Test 6").to.equal(6);
  ...
  describe('C++14 parser:', function () {
    it('Simple C++ example', function (done) {
    ...
    it('Simple C++ example with errors in input', function (done) {
      let inputStream = new ANTLRInputStream("class A {\n" +
        "public:\n" +
        "  void test() {\n" +
        "    if ()" +
